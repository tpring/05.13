  ...
  return (
    <>
      ...
      <ul>
        ...
        {users.map( users => (
          <div style={{ display: 'flex', justifyContent: 'center', wordSpacing: '5px'}}>
          <p style={{ marginRight: '10px' }}>이름 : {users.name}  나이 : {users.age}</p>
          <button onClick={() => removeUser(users.id)}>삭제</button>
          </div>
        ))}
      </ul>
    </>
